...
function startAscend() {
  ...
  hidePanels();
  if (gameMode==='classic') cSpeed = Math.max(55, cSpeed - 22);
...
function classicTick() {
  ...
  // Speed scaling — every 5 apples
  if (apples>0 && apples%5===0 && cSpeed>CFG.MIN_SPEED) {
    cSpeed = Math.max(CFG.MIN_SPEED, cSpeed - CFG.SPEED_STEP);
...
function freeAimUpdate() {
  ...
  const spd = CFG.FA_SPEED * (ascendMode?1.55:1);
